...
def ari_per_word_weighted(df):
    ...
    return cumsum / total, np.mean([ari for ari, count in words.values()]), words


def evaluate(dataset_fpath):
    df = read_csv(dataset_fpath, sep='\t', dtype={'gold_sense_id': str, 'predict_sense_id': str})
    weightedari, meanari, words = ari_per_word_weighted(df)
    print('{}\t{}\t{}'.format('word', 'ari', 'count'))

    for word in sorted(words.keys()):
        print('{}\t{:.6f}\t{:d}'.format(word, *words[word]))

    print('Weighted ari:\t{:.6f}\t{:d}'.format(weightedari, len(df)))
    print('Mean ari:\t{:.6f}\t{:d}'.format(meanari, len(df)))
...
def main():
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter,
                                     description='Evaluate the quality of Word Sense Induction (WSI) '
                                                 ...
                                                 '"data" directory (train.tsv). To run an already pre-filled '
                                                 'baseline system run '
