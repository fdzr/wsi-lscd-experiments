...
def connected_components_clustering(
    ...
    edges_negative = [
        (i, j) for (i, j) in _graph.edges()
        if _graph[i][j]['weight'] < positive_threshold or is_non_value(_graph[i][j]['weight'])
    ]
...
def chinese_whispers_clustering(
    ...
    _cw_clustering = cw.aggregate_clusters(
        cw.chinese_whispers(_graph, weighting=weighting, iterations=iterations, seed=seed)
    )
...
def _adjacency_matrix_to_nxgraph(adjacency_matrix: np.ndarray) -> nx.Graph:
...
def _convert_graph_cluster_list_set_to_list(graph: nx.Graph, cluster_list: list) -> list:
...
if __name__ == '__main__':
    ...
    print(clusters)
    print(_convert_graph_cluster_list_set_to_list(graph=G, cluster_list=clusters))
